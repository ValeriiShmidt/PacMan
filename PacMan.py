...
class Player:
    ...
    def grab_coin(self):
        ...
        if self.application.coins:
            ...
            if self.target_coin in coins:
                path = AAlg(self.application.grid_map, (int(self.grid_pos[1]), int(self.grid_pos[0])),
                            (int(self.target_coin[0]), int(self.target_coin[1])), manhattan, 0)
                ...
                distances = [(manhattan(self.grid_pos, coin, 0), coin) for coin in coins]
                print(distances)
                print("position", self.grid_pos)
    ...
    def get_score(self, direction):
        ...
        distance_to_enemy = manhattan(new_step, enemy, 0)

        distances = []
        for coin in self.application.coins:
            distances.append((manhattan(coin, self.grid_pos, 0), coin))

        distance_and_coin = min(distances, key=lambda x: x[0])
        self.target_coin = distance_and_coin[1][::-1]
        new_step = (int(self.grid_pos[0] + direction[0]), int(self.grid_pos[1] + direction[1]))
        if manhattan(new_step, distance_and_coin[1], 0) < distance_and_coin[0]:
    ...
    def way_through_4_points(self):
        ...
        for j in range(len(points) - 1):
            ...
            for i in range(j + 1, len(points)):
                ...
                temp_routs.append(AAlg(self.application.grid_map, point1, point2, euclid, 0))
